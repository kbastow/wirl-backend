...
// PUT - add to savedBlogs --------------------------------------
router.put('/addSavedBlogs/', Utils.authenticateToken, (req, res) => {  
  // validate check
  if(!req.body.blogId){
  ...
  // add blogId to savedBlogs field (array - push)
  User.updateOne({
    _id: req.user._id
  }, {
    $push: {
      savedBlogs: req.body.blogId
    }
  })
    .then((user) => {            
      res.json({
        message: "Blog saved to list"
      })
    })
    .catch(err => {
      console.log(err)
...
router.get('/:id', Utils.authenticateToken, (req, res) => {
  ...
  User.findById(req.params.id).populate('savedBlogs')
    .then(user => {
      res.json(user)
    })
    .catch(err => {
